...
//
// ----------------------------------------------
// SECCION          : CAPEX - GESTIONES NO APROBADAS//
...
FNReconsiderarSponsor = function (token) {
    var usuario = $("#CAPEX_H_USERNAME").val();
    $.ajaxSetup({ cache: false });

    $.ajax({
        url: "GestionNoAprobada/ReconsiderarIniciativa",
        method: "POST",
        data: { "Usuario": usuario, "IniToken": token }
    }).done(function (r) {
        var obj = JSON.parse(JSON.stringify(r));
        if (obj.Mensaje == "Reconsiderada") {
            swal("", "Iniciativa Reconsiderada y devuelta a Visación.", "success");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
        else {
            swal("", "Problemas al intentar reconsiderar iniciativa.", "error");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
...
FNFilterActionGetData = function () {
    ...
    $.ajax({
        ...
        if (r && r.success && r.tableTrs) {
            ...
            if (opcionEnviada == "3") {
                $("#ContenedorContNoAprobadaAMSAIni").html(r.countBadge);
            } else if (opcionEnviada == "2") {
                $("#ContenedorContNoAprobadaCEIni").html(r.countBadge);
            } else {
                $("#ContenedorContNoAprobadaGAFIni").html(r.countBadge);
            }
        } else {
            $("#paginador").html('');
            if (opcionEnviada == "3") {
...
FNFilterData = function () {
    ...
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionNoAprobada/getTreeFilter?opcionEnviada=" + opcionEnviada,
        method: "GET"
    }).done(function (r) {
        //var obj = JSON.parse(JSON.stringify(r));
        //console.log('done', obj);
...
FNReintegrarIniciativaDesarrollo = function (token) {
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionNoAprobada/ReintegrarIniciativaDesarrollo",
        method: "POST",
        data: { "IniToken": token }
    }).done(function (r) {
        //PARSEAR RESPUESTA
        ...
        //PROCESAR RESPUESTA
        if (estado == "Error") {
            swal("Error", "No es posible reintegrar esta inicativa.", "error");
            setTimeout(function () {
                document.location.reload();
            }, 3000);
        }
        else if (estado == "Reintegrada") {
            swal("Exito", "Iniciatica reintegrada a desarrollo.", "success");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
...
FNReintegrarIniciativaVisacion = function (token) {
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionNoAprobada/ReintegrarIniciativaVisacion",
        method: "POST",
        data: { "IniToken": token }
    }).done(function (r) {
        //PARSEAR RESPUESTA
        ...
        if (estado == "Error") {
            ...
            swal("Exito", "Iniciatica reintegrada a visación.", "success");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
...
FNReintegrarIniciativaVisacionAMSA = function (token) {
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionNoAprobada/ReintegrarIniciativaVisacionAMSA",
        method: "POST",
        data: { "IniToken": token }
    }).done(function (r) {
        //PARSEAR RESPUESTA
