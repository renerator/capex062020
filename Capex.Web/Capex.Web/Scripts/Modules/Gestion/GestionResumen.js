...
FNEliminarIniciativa = function (token) {
    ...
    $.ajax({
        url: "GestionResumen/EliminarIniciativa",
        method: "POST",
        data: { "IniToken": token, "Usuario": usuario }
    }).done(function (r) {
        var obj = JSON.parse(JSON.stringify(r));
        if (obj.Mensaje == "Eliminada") {
            swal("", "Iniciativa Eliminada.", "success");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
        else {
            swal("", "Problemas al eliminar iniciativa.", "error");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
...
FNFilterActionGetData = function () {
    ...
    $.ajax({
        ...
        if (r && r.success && r.tableTrs) {
            ...
            $("#ContenedorContResumenIni").html(r.countBadge);
        } else {
            $("#paginador").html('');
...
FNFilterData = function () {
    ...
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionResumen/getTreeFilter",
        method: "GET"
    }).done(function (r) {
        //var obj = JSON.parse(JSON.stringify(r));
        //console.log('done', obj);
...
$("#buscarConFiltros").click(function () {
    console.log('buscarConFiltros click');
    $.ajax({
        type: "GET",
        url: "GestionResumen/ObtenerDatosConFiltros",
        dataType: "json",
        data: {
            AreaToken: Area.val(),
        ...
        success: function (resp) {
            console.log(resp);
            var content = '<tbody>' + resp + '</tbody>';
            $('#iniciativas tbody').replaceWith(content);
        }
    });
});
