...
//
// ----------------------------------------------
// SECCION          : CAPEX MODULO - VISTA INICIATIVAS COMENTADAS
//
...
FNGuardarComentario = function () {
    var Token = localStorage.getItem("CAPEX_GESTION_INICIATIVA_TOKEN");
    var Usuario = localStorage.getItem("CAPEX_GESTION_INICIATIVA_USUARIO");
    ...
    if (Comentario == "" || Comentario == null) {
        ...
        var DTO = {
            "IniToken": Token,
            "ObsRemite": Usuario,
            "ComeTexto": Comentario,
            "ComePrioridad": Prioridad
        }
        $.ajax({
            url: "GestionComentada/Guardar",
            type: "POST",
            dataType: "json",
            data: (DTO),
            success: function (r) {
                ...
                //PROCESAR RESPUESTA
                if (estado == "Error") {
                    swal("Error", "No es posible guardar el comentario.", "error");
                }
...
//
FNCerrarModalComentar = function () {
    $("#ModalComentar").hide();
    setTimeout(function () {
        window.location.reload(true);
    }, 300)
...
//
// VER TODAS LAS INICIATIVAS CON COMENTARIOS
//

//
// VER LOS COMENTARIOS DE UNA INICIATIVA
//

//
// RESPONDER COMENTARIO DE UNA INICIATIVA
//
...
//
// ADJUNTAR ARCHIVO EN UN COMENTARIO
//

//
// DESCARGAR ARCHIVO DE UN COMENTARIO
//
...
FNFilterActionGetData = function () {
    ...
    $.ajax({
        ...
        if (r && r.success && r.tableTrs) {
            ...
            $("#ContenedorContComent").html(r.countBadge);
        } else {
            $("#paginador").html('');
...
FNFilterData = function () {
    ...
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionComentada/getTreeFilter",
        method: "GET"
    }).done(function (r) {
        //var obj = JSON.parse(JSON.stringify(r));
        //console.log('done', obj);
...
FNObtenerUltimoComentario = function (PidToken) {
    console.log("FNObtenerUltimoComentario PidToken=", PidToken);
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "Planificacion/ObtenerUltimaObservacionVisacion",
        method: "POST",
        data: { "PidToken": PidToken }
    }).done(function (r) {
        ...
        console.log('done estructura', estructura);
        estructura = estructura.Mensaje;
        //DESCOMPONER RESPUESTA
        var parte = estructura.split("|");
        var obtenido = parte[0];
        var estado = parte[1];
        var comentario = parte[2];
        var extra = parte[3];
        if (obtenido == "Obtenido") {
            $('#ObservacionRechazoAdmin1').val(comentario);
            $('#ModalObservacionRechazoAdmin1').show();
        } else if (obtenido == "Error") {
            $('#ObservacionRechazoAdmin1').val('');
            $('#ModalObservacionRechazoAdmin1').hide();
        }

    }).fail(function (xhr) {
        $('#ObservacionRechazoAdmin1').val('');
        $('#ModalObservacionRechazoAdmin1').hide();
        console.log('FNObtenerUltimoComentario fail');
        console.log('error', xhr);
    });
}


FNCerrarObservacionRechazoAdmin1 = function () {
    FNLimpiarSelect();
    $('#ModalObservacionRechazoAdmin1').hide();
}
...
FNAsignarmela = function (PidToken) {
    console.log("FNAsignarmela PidToken=", PidToken);
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "Planificacion/AsignarmeIniciativaDesdeObservadas",
        method: "POST",
        data: { "PidToken": PidToken }
    }).done(function (r) {
        if (r && r.redirectUrlLogout && r.redirectUrlLogout == "true") {
        ...
        var estructura = JSON.parse(JSON.stringify(r));
        console.log('done FNAsignarmela estructura', estructura);
        estructura = estructura.Mensaje;
        //DESCOMPONER RESPUESTA
        var parte = estructura.split("|");
        var asignado = parte[0];
        var estado = parte[1];
        var comentario = parte[2];
        var extra = parte[3];
        if (asignado == "Asignado") {
            swal("Exito", "Solicitud asignada exitosamente.", "success");
            setTimeout(function () {
                window.location.href = location.protocol + '//' + location.host + "/GestionObservada";
            }, 1500)
        } else if (asignado == "Error") {
            FNLimpiarSelect();
            if (estado && estado != undefined && estado != "") {
                if (estado == "1" || estado == "2") {
                    swal("Error", comentario, "error");
                } else {
                    swal("Error", "Error al asignarme la iniciativa.", "error");
                }
            } else {
                swal("Error", "Error al asignarme la iniciativa.", "error");
            }
        ...
        console.log('FNAsignarmela fail');
        console.log('error', xhr);
    });
}
