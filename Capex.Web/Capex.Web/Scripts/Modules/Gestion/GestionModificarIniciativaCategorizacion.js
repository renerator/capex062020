...
FNEvaluarRequerimientoNivelIngeneria = function (CatCategoria, idenSuperIntendencia) {
    var categoria = CatCategoria;
    if (!categoria || categoria == undefined || categoria == "" || categoria == "-1" || categoria < 0) {
        categoria = $('#Categoria').val();
    }
    var superIntendencia = idenSuperIntendencia;
    if (!superIntendencia || superIntendencia == undefined || superIntendencia == "" || superIntendencia == "-1" || superIntendencia < 0) {
        superIntendencia = $('#Superitendencia').val();
    }
    console.log("FNEvaluarRequerimientoNivelIngeneria categoria=" + categoria + ", superIntendencia=" + superIntendencia);
...
FNEvaluarCategoriaCargar = function (CatCategoriaToken, IdenSuperIntendencia, NINombre, NIAcronimo) {
    setTimeout(function () {
        console.log("FNEvaluarCategoriaCargar CatCategoriaToken=" + CatCategoriaToken + ", NINombre=" + NINombre + ", NIAcronimo=" + NIAcronimo);
        if (CatCategoriaToken && CatCategoriaToken != "" && CatCategoriaToken != "NO DISPONIBLE") {
            $('#Categoria').val(CatCategoriaToken);
        }
        localStorage.setItem("CAPEX_NIVEL_INGENIERIA_ALTERNATIVO", "");
        if (FNEvaluarRequerimientoNivelIngeneria(CatCategoriaToken, IdenSuperIntendencia)) {
            $('#ContenedorNivelIngenieria1').hide();
            $('#ContenedorNivelIngenieria2').show();
            var contentSpan = "<span><strong>" + NINombre + "</strong></span>";
            $("#ContenedorNivelIngenieria2").html(contentSpan);
            localStorage.setItem("CAPEX_NIVEL_INGENIERIA_ALTERNATIVO", NIAcronimo);
        } else {
...
FNEvaluarCategoriaCargarCorregido = function (CatCategoriaToken, IdenSuperIntendencia, NINombre, NIAcronimo) {
    setTimeout(function () {
        console.log("FNEvaluarCategoriaCargarCorregido CatCategoriaToken=" + CatCategoriaToken + ", NINombre=" + NINombre + ", NIAcronimo=" + NIAcronimo);
        if (CatCategoriaToken && CatCategoriaToken != "" && CatCategoriaToken != "NO DISPONIBLE") {
            $('#Categoria').val(CatCategoriaToken);
        }
        if (FNEvaluarRequerimientoNivelIngeneria(CatCategoriaToken, IdenSuperIntendencia)) {
            FNObtenerListadoNivelIngenieraNoRequiereCorregido(true, NIAcronimo, NINombre);
        } else {
            FNObtenerListadoNivelIngenieraNoRequiereCorregido(false, NIAcronimo, NINombre);
        }
    }, 1800);
}

FNObtenerListadoNivelIngenieraNoRequiereCorregido = function (append, keyParam, valueParam) {
    var DTO = {};
    $.ajax({
        url: "../../Planificacion/ListarNivelIngenieriaNoRequiere",
        type: "GET",
...
FNEvaluarCategoria = function () {
    ...
    if (FNEvaluarRequerimientoNivelIngeneria("", "")) {
        $('#ContenedorNivelIngenieria1').hide();
        $('#ContenedorNivelIngenieria2').show();
        FNObtenerListadoNivelIngenieraNoRequiere();
    } else {
        FNMostrarDefaultNivelIngenieria(false);
    }
}

FNEvaluarCategoriaCorregido = function () {
    if (FNEvaluarRequerimientoNivelIngeneria("", "")) {
    ...
    if (Categoria.trim() == "9005BD87-6800-49F1-9D68-9A11EFBBDBAF") {//HSEC
        console.log("FNEvaluarCategoriaCorregido if Categoria=", Categoria);
        FNObtenerListadoClasificacionSSOHSEC();
    } else {
        console.log("FNEvaluarCategoriaCorregido else Categoria=", Categoria);
...
// LISTAR OPCIONES SSO
//
FNObtenerListadoClasificacionSSOHSEC = function (CSSOToken) {
    //PREPARAR
    ...
    $.ajax({
        ...
        success: function (r) {
            ...
            setTimeout(function () {
                ...
                $.each(JSON && JSON.parse(r) || $.parseJSON(r), function (key, value) {
                    if (value.CSToken != "029FA2E7-58FA-4E86-B93D-8ED5A9DA28ED") {//EXCEPTO NO APLICA
                        if (CSSOToken && CSSOToken != undefined && CSSOToken.trim() != "" && CSSOToken.trim() == value.CSToken) {
                            ClasificacionSSO.append(new Option(value.CSNombre, value.CSToken, false, true));
                            match = true;
                        } else {
                ...
                if (cuantos == 1) {
                    ...
                    if (!match) {
                        ...
                        ClasificacionSSO.append('<option value="-1">Seleccionar..</option>')
                    }
                }
            }, 200);
        }
...
//
// REFACTORIZACION DE METODODS
//
FNProcesarEstadoMacroCategoria = function (caso, valor, contenedor, token) {
    console.log("FNProcesarEstadoMacroCategoria caso=" + caso + ", valor=" + valor + ", contenedor=" + contenedor + ", token=" + token);
    ...
    if (caso == "INGRESO") {
        if (valor == "SI") {
            ...
            localStorage.setItem("CAPEX_CLASIFICA_MACRO", contenedor);
            FNObtenerListadoCategorias("DEVELOPMENT", token);
            //FNMostrarDefaultNivelIngenieria(false);
            FNObtenerListadoNivelIngenieraNoRequiereCorregido(false, "", "");
            return;
        } else if (valor == "NO") {
            ...
            localStorage.setItem("CAPEX_CLASIFICA_MACRO", contenedor);
            FNObtenerListadoCategorias("SUSTAINING", token);
            return;
        } else {
            $("#RequiereCapacidad1").prop('checked', false);
            $("#RequiereCapacidad2").prop('checked', false);
        }
    } else if (caso == "MODIFICACION") {
        if (valor == "SI") {
            $("#RequiereCapacidad1").prop('checked', true);
            $("#RequiereCapacidad2").prop('checked', false);
            $('#ContenedorMacro2').hide();
            $('#ContenedorMacro1').show();
            localStorage.setItem("CAPEX_CLASIFICA_MACRO", contenedor);
            FNObtenerListadoCategorias("DEVELOPMENT", token, "0");
            return;
        } else if (valor == "NO") {
            $("#RequiereCapacidad1").prop('checked', false);
            $("#RequiereCapacidad2").prop('checked', true);
            $('#ContenedorMacro1').hide();
            $('#ContenedorMacro2').show();
            localStorage.setItem("CAPEX_CLASIFICA_MACRO", contenedor);
            FNObtenerListadoCategorias("SUSTAINING", token, "0");
            return;
        } else {
            $("#RequiereCapacidad1").prop('checked', false);
            $("#RequiereCapacidad2").prop('checked', false);
...
//
FNObtenerListadoCategorias = function (Macrocategoria, token, asyncCallParam) {
    console.log("FNObtenerListadoCategorias Macrocategoria=" + Macrocategoria + ", token=" + token + ", asyncCallParam=" + asyncCallParam);
    //PREPARAR
    var asyncCall = false;
    if (!asyncCallParam || asyncCallParam == undefined || asyncCallParam == "") {
        asyncCall = true;
    }
    console.log("FNObtenerListadoCategorias luego de evaluada Macrocategoria=" + Macrocategoria + ", token=" + token + ", asyncCall=" + asyncCall + ", asyncCallParam=" + asyncCallParam);
    ...
    $.ajax({
        ...
        success: function (r) {
            ...
            setTimeout(function () {
                    ...
                    if (token != "" || token != null) {
                        ...
                        if (Macrocategoria == "DEVELOPMENT") {
                            if (value.CatToken.trim() == "BDC8CC53-16CB-4F82-8B83-79B02C323EDE" || value.CatToken.trim() == "0D58C515-9D3C-4A02-B904-A5D04310EB61" || value.CatToken.trim() == "1D823A68-B601-4763-B91D-4F28571DF372" || value.CatToken.trim() == "30A15475-6851-4DCB-861C-43106284E241") {
                                if (value.CatToken.trim() === token.trim()) {
                                    Categoria.append(new Option(value.CatNombre, value.CatToken, false, true));
                                } else {
                ...
                console.log("FNObtenerListadoCategorias Fin Macrocategoria=" + Macrocategoria + ", token=" + token);
            }, 200);
        }
    });
}
...
//
FNObtenerListadoNivelIngeniera = function (token) {
    console.log("FNObtenerListadoNivelIngeniera token=", token);
    //PREPARAR
    var NivelIngenieria = $('#NivelIngenieria');
    NivelIngenieria.empty();
    NivelIngenieria.append('<option selected="true">Buscando..</option>');
    //LISTO
    var DTO = {};
    var cuantos = 0;
    var match = false;
            ...
            setTimeout(function () {
                ...
                $.each(JSON && JSON.parse(r) || $.parseJSON(r), function (key, value) {
                    if (token != "" || token != null || token != undefined) {
                        //MODIFICACION
                        if (token.trim() == value.NIAcronimo.trim()) {
                            NivelIngenieria.append(new Option(value.NINombre, value.NIAcronimo, false, true));
                            match = true;
                        } else {
                            NivelIngenieria.append(new Option(value.NINombre, value.NIAcronimo, false, false));
                        }
                ...
                if (cuantos == 1) {
                    ...
                    if (!match) {
                        ...
                        NivelIngenieria.append('<option value="-1">Seleccionar..</option>')
                    }
                }
                console.log("FNObtenerListadoNivelIngeniera fin token=", token);
            }, 200);
        }
    });
}
...
FNObtenerListadoClasificacionSSO = function (CSToken) {
    console.info("GestionModificarIniciativaCategorizacion.js FNObtenerListadoClasificacionSSO CSToken=", CSToken);
    //PREPARAR
    var ClasificacionSSO = $('#ClasificacionSSO');
    ClasificacionSSO.empty();
    ClasificacionSSO.append('<option  value="-1" selected="true">Buscando..</option>');
...
FNProcesarOpcionSSO = function () {
    ...
    console.log("FNProcesarOpcionSSO glosa=", glosa);
...
//
FNObtenerEstandarSeguridad = function (token, token_es) {
    //PREPARAR
    console.info("GestionModificarIniciativaCategorizacion.js FNObtenerEstandarSeguridad token=" + token);
    console.info("GestionModificarIniciativaCategorizacion.js FNObtenerEstandarSeguridad token_es=" + token_es);
    ...
    //LISTO
    var EssComToken = 'F1DBFB20-6DCA-4537-A66E-53E3B3C3B830';
    var DTO = { "EssComToken": EssComToken, "EssCSToken": token };
    var cuantos = 0;
    var match = false;
    $.ajax({
        ...
        success: function (r) {
            ...
            setTimeout(function () {
                ...
                $.each(JSON && JSON.parse(r) || $.parseJSON(r), function (key, value) {
                    if (token_es == value.EssToken) {
                        EstandarSeguridad.append(new Option(value.EssNombre, value.EssToken, false, true));
                        match = true;
                    } else {
                ...
                if (cuantos == 1) {
                    ...
                    if (!match) {
                        ...
                        EstandarSeguridad.append('<option  value="-1">Seleccionar..</option>')
                    }
                }
            }, 200);
        }
...
//
// ESTANDAR DE SEGURIDAD /INGRESO
//
FNObtenerEstandarSeguridadSeleccionable = function (token) {
    var EstandarSeguridad = $('#EstandarSeguridad');
    EstandarSeguridad.empty();
    EstandarSeguridad.append('<option  value="-1" selected="true">Buscando..</option>');
    var EssComToken = 'F1DBFB20-6DCA-4537-A66E-53E3B3C3B830';
    ...
    $.ajax({
        ...
        success: function (r) {
            ...
            if (token != "" || token != undefined || token != "NO DISPONIBLE") {
                if (cuantos == 1) {
                    EstandarSeguridad.prop('selectedIndex', 0);
                } else if (cuantos > 1) {
                    EstandarSeguridad.append('<option value="-1" selected="true">Seleccionar..</option>')
...
FNGuardarCategorizacion = function () {
    ...
    if (!FNCheckFormDinamyc()) {
        ...
        var EstandarSeguridad = $("#EstandarSeguridad").val();
        ...
        if (EstadoProyecto == "-1" || EstadoProyecto == "" || CatMacroCategoria == "" || Categoria == "-1" || Categoria == "" || CatAgrega == "" || CatClase == "-1" || CatClase == "" || ClasificacionSSO == "" || EstandarSeguridadAux == "-1" || EstandarSeguridadAux == "" || NivelIngenieria == "" || NivelIngenieria == "-1") {
...
FNActualizarCategorizacion = function () {
    var estado = localStorage.getItem("CAPEX_INICIATIVA_CATEGORIZACION_ESTADO");
    var IniToken = localStorage.getItem("CAPEX_INICIATIVA_TOKEN");
    console.log("FNActualizarCategorizacion IniToken=", IniToken);
    if (IniToken == "") {
    ...
    if (!FNCheckFormDinamyc()) {
        ...
        if (EstadoProyecto == "-1" || EstadoProyecto == "" || CatMacroCategoria == "" || Categoria == "-1" || Categoria == "" || CatAgrega == "" || CatClase == "-1" || CatClase == "" || ClasificacionSSO == "" ||
            ClasificacionSSO == "-1" || EstandarSeguridadAux == "-1" || EstandarSeguridadAux == "" || NivelIngenieria == "" || NivelIngenieria == "-1") {
