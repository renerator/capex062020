...
// ----------------------------------------------
//
// ELIMINAR INICIATIVA EN DESARROLLO
//
...
FNEliminarIniciativa = function (token) {
    ...
    $.ajax({
        url: "GestionDesarrollo/EliminarIniciativa",
        method: "POST",
...
FNFilterActionGetData = function (load) {
    ...
    $.ajax({
        ...
        if (r && r.success && r.tableTrs) {
            ...
            $("#ContenedorContDesaIni").html(r.countBadge);
        } else {
            $("#paginador").html('');
...
FNFilterData = function () {
    ...
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionDesarrollo/getTreeFilter",
        method: "GET"
    }).done(function (r) {
        //var obj = JSON.parse(JSON.stringify(r));
        //console.log('done', obj);
