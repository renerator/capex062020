...
FNAprobarIniciativaSponsor = function (token) {
    var usuario = $("#CAPEX_H_USERNAME").val();
    $.ajaxSetup({ cache: false });

    $.ajax({
        url: "GestionVisacion/AprobarIniciativa",
        method: "POST",
        ...
        if (obj.Mensaje == "Aprobada") {
            ...
            swal("", "Problemas al intentar aprobar iniciativa.", "error");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
...
// RECHAZO INICIATIVA
//
FNRechazoIniciativaSponsor = function (token) {
    var token = localStorage.getItem("CAPEX_GESTION_INICIATIVA_TOKEN");
    var usuario = $("#CAPEX_H_USERNAME").val();
    var observacion = $("#ObservacionRechazoSponsor").val();

    $.ajaxSetup({ cache: false });

    $.ajax({
        url: "GestionVisacion/RechazarIniciativa",
        method: "POST",
        ...
        if (obj.Mensaje == "Rechazada") {
            ...
            swal("", "Problemas al intentar rechazar iniciativa.", "error");
            setTimeout(function () {
                window.location.reload(true);
            }, 3000);
        }
...
//
// ABRIR VENTANA RECHAZO
//
FNAbrirRechazoSponsor = function (iniciativa) {
    localStorage.setItem("CAPEX_GESTION_INICIATIVA_TOKEN", iniciativa);
    $("#ModalRechazoSponsor").show();
    return false;
}
//
// CERRAR VENTANA RECHAZO
//
FNCerrarRechazoSponsor = function () {
    $("#ModalRechazoSponsor").hide();
    setTimeout(function () {
        window.location.reload(true);
    }, 300);
}
...
FNFilterData = function () {
    ...
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionVisacion/getTreeFilter",
        method: "GET"
    }).done(function (r) {
        //var obj = JSON.parse(JSON.stringify(r));
        //console.log('done', obj);
...
FNObtenerUltimoComentarioCE = function (PidToken) {
    console.log("FNObtenerUltimoComentarioCE PidToken=", PidToken);
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "Planificacion/ObtenerUltimaObservacionComiteCE",
        method: "POST",
...
FNObtenerUltimoComentarioCAMSA = function (PidToken) {
    console.log("FNObtenerUltimoComentarioCAMSA PidToken=", PidToken);
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "Planificacion/ObtenerUltimaObservacionComiteAMSA",
        method: "POST",
        data: { "PidToken": PidToken }
    }).done(function (r) {
        if (r && r.redirectUrlLogout && r.redirectUrlLogout == "true") {
...
FNLimpiarSelect = function () {
    console.log("FNLimpiarSelect desde Visacion");
    setTimeout(function () {
...
FNNoAprobarIniciativaVisacion = function (token) {
    $.ajaxSetup({ cache: false });
    $.ajax({
        url: "GestionVisacion/NoAprobarIniciativaVisacion",
        method: "POST",
        data: { "IniToken": token }
    }).done(function (r) {
        //PARSEAR RESPUESTA
