...
FNFormatearReglaUnoEvalRi = function (nStr) {
    nStr += '';
    x = nStr.split('.');
    x1 = x[0];
    x2 = x.length > 1 ? '.' + x[1] : '';
...
/*document.getElementById("EvrMFL1").onblur = function () {
...
FNPreviousCallBackIngresoEvaluacionRiesgoCheckToken = function () {
...
FNPreviousCallBackIngresoEvaluacionRiesgo = function () {
    console.log("FNPreviousCallBackIngresoEvaluacionRiesgo");
    jQuery("#AppLoaderContainer").show();
}

FNCallBackIngresoEvaluacionRiesgo = function (paramJson) {
    jQuery("#AppLoaderContainer").hide();
    if (paramJson.Data.code == "0") {
        swal("Exito", "Archivo evaluacion riesgo subido correctamente", "success");
        FNPoblarDocumentos();
    } else {
        swal("Error", "Error al intentar subir archivo de Evaluacion Riesgo.", "error");
    }
}


FNObtenerMatrizRiesgo = function () {
    var DTO = {};
    $.ajax({
...
//
/*FNResolverRiesgo = function () {
    var t = setInterval(FNCalculoRiesgoSinProyecto, 1000);
    var t = setInterval(FNCalculoRiesgoConProyecto, 1000);
}*/
