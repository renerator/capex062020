...
namespace Capex.Web.Controllers
{
    ...
    [RoutePrefix("GestionNoAprobada")]
    public class GestionNoAprobadaController : Controller
    {
        /* ------------------------------------------------------------------------------------
         * 
         * PMO360
        ...
        ///     
        ///     ----------------------------------------------------------------------------
        ///     CONTROLADOR "GestionNoAprobadaController" 
        ///     VERSION     0.0.1
        ///     ----------------------------------------------------------------------------
        ///     PROPOSITO
        ///     ----------------------------------------------------------------------------
        ...
        #region "CONSTRUCTOR"
        public GestionNoAprobadaController()
        {
            ////IDENTIFICACION
        ...
        public ActionResult Index(string opcion)
        {
            ...
            else
            {
                ...
                Session["CAPEX_SESS_MENU_ACTIVO_OPCION1"] = "";
                Session["CAPEX_SESS_MENU_ACTIVO_OPCION2"] = "";
                Session["CAPEX_SESS_MENU_ACTIVO_OPCION3"] = "";
                Session["CAPEX_SESS_OPCION_ENVIADA"] = "1";
                ...
                else
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(opcion) || "1".Equals(opcion))
                        {
                            Session["CAPEX_SESS_OPCION_ENVIADA"] = "1";
                            Session["CAPEX_SESS_MENU_ACTIVO_OPCION1"] = "active";
                            Session["CAPEX_SESS_VISTA_CONTENEDORA_PADRE"] = "GestionNoAprobadaGAF";
                            if (tipoIniciativaSeleccionado.Equals("0"))
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_GAF", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_GAF_2", new { @usuario = usuarioAux, @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa != null && Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        {
                            Session["CAPEX_SESS_OPCION_ENVIADA"] = "2";
                            Session["CAPEX_SESS_MENU_ACTIVO_OPCION2"] = "active";
                            Session["CAPEX_SESS_VISTA_CONTENEDORA_PADRE"] = "GestionNoAprobadaCE";
                            if (tipoIniciativaSeleccionado.Equals("0"))
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_CE", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa != null && Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_CE_2", new { @usuario = usuarioAux, @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa != null && Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        {
                            Session["CAPEX_SESS_OPCION_ENVIADA"] = "3";
                            Session["CAPEX_SESS_MENU_ACTIVO_OPCION3"] = "active";
                            Session["CAPEX_SESS_VISTA_CONTENEDORA_PADRE"] = "GestionNoAprobadaAMSA";
                            if (tipoIniciativaSeleccionado.Equals("0"))
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_AMSA", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa != null && Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_AMSA_2", new { @usuario = usuarioAux, @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa != null && Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        /*//SPONSOR
                        if (rol.Contains("Sponsor"))
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_SPONSOR", new { @usuario = usuario }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        else if (rol != "Sponsor" && opcion == "Sponsor")
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_SPONSOR", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        //ADMIN1
                        if (rol.Contains("Administrador1"))
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_ADMIN1", new { @usuario = usuario }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        else if (rol != "Administrador1" && opcion == "Administrador1")
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_ADMIN1", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        //ADMIN2
                        if (rol.Contains("Administrador2"))
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_ADMIN2", new { @usuario = usuario }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        else if (rol != "Administrador2" && opcion == "Administrador2")
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_ADMIN2", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        //ADMIN3
                        if (rol.Contains("Administrador3"))
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_ADMIN3", new { @usuario = usuario }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }
                        else if (rol != "Administrador1" && opcion == "Administrador3")
                        {
                            ViewBag.Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_ADMIN3", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                            Session["CAPEX_SESS_MENU_ACTIVO"] = "Active";
                        }*/
                    }
                    catch (Exception ex)
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult getTreeFilter(string opcionEnviada)
        {
            if (!@User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Logout", "Login");
            ...
            else
            {
                ...
                {
                    var paginaItem = 9;
                    if (string.IsNullOrEmpty(opcionEnviada) || "1".Equals(opcionEnviada))
                    {
                        paginaItem = 9;
                    }
                    else if ("2".Equals(opcionEnviada))
                    {
                        paginaItem = 10;
                    }
                    else
                    ...
                    var tipoIniciativaSeleccionado = Convert.ToString(Session["tipoIniciativaSeleccionado"]);
                    var categorias = ORM.Query("CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = paginaItem, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                    var items = ORM.Query("[CAPEX_SEL_CATEGORIA_FILTRO_ITEM_VIGENTES]", new { @pagina = paginaItem, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
        ...
        [HttpPost]
        public ActionResult getData(string filtroGetData, string opcionEnviada)
        {
                    ...
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(opcionEnviada) || "1".Equals(opcionEnviada))
                        {
                            if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                            {
                                ...
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_GAF_2_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            else
                            {
                                if (tipoIniciativaSeleccionado.Equals("0"))
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_GAF_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                                ...
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_GAF_2_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            if (ViewBag.Iniciativas != null && ViewBag.Iniciativas.Count > 0)
                            {
                                ...
                                else if (Convert.ToString(@Session["CAPEX_SESS_ROLNOMBRE"]) == "Administrador1")
                                {
                                    ...
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "2" + Convert.ToChar(34) + ">Adjuntos</option>");
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "3" + Convert.ToChar(34) + ">Pdf</option>");
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "20" + Convert.ToChar(34) + ">Reintegrar</option>");
                                    selectTrs.Append("</select>");
                                }
                        ...
                        else if ("2".Equals(opcionEnviada))
                        {
                            if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                            {
                                if (tipoIniciativaSeleccionado.Equals("0"))
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_CE_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                                ...
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_CE_2_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            else
                            {
                                if (tipoIniciativaSeleccionado.Equals("0"))
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_CE_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                                ...
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_CE_2_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            if (ViewBag.Iniciativas != null && ViewBag.Iniciativas.Count > 0)
                            {
                                ...
                                else if (Convert.ToString(@Session["CAPEX_SESS_ROLNOMBRE"]) == "Administrador2")
                                {
                                    ...
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "2" + Convert.ToChar(34) + ">Adjuntos</option>");
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "3" + Convert.ToChar(34) + ">Pdf</option>");
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "21" + Convert.ToChar(34) + ">Reintegrar</option>");
                                    selectTrs.Append("</select>");
                                }
                                else if (Convert.ToString(@Session["CAPEX_SESS_ROLNOMBRE"]) == "Administrador3")
                                {
                        ...
                        else
                        {
                            if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                            {
                                if (tipoIniciativaSeleccionado.Equals("0"))
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_AMSA_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            else
                            {
                                if (tipoIniciativaSeleccionado.Equals("0"))
                                {
                                    var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_NO_APROBADA_AMSA_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            if (ViewBag.Iniciativas != null && ViewBag.Iniciativas.Count > 0)
                            {
                                ...
                                else if (Convert.ToString(@Session["CAPEX_SESS_ROLNOMBRE"]) == "Administrador3")
                                {
                                    ...
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "2" + Convert.ToChar(34) + ">Adjuntos</option>");
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "3" + Convert.ToChar(34) + ">Pdf</option>");
                                    selectTrs.Append("<option value=" + Convert.ToChar(34) + "22" + Convert.ToChar(34) + ">Reintegrar</option>");
                                    selectTrs.Append("</select>");
                                }
                                foreach (var result in ViewBag.Iniciativas)
                                {
        ...
        /// <summary>
        /// METODO PARA RECONSIDERACION DE INICITAIVA
        /// </summary>
        /// <param name="IniToken"></param>
        /// <param name="Usuario"></param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult ReconsiderarIniciativa(string IniToken, string Usuario)
        {
            try
            {
                ...
                ORM.Query("CAPEX_UPD_GESTION_RECONSIDERAR", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
                return Json(new { Mensaje = "Reconsiderada" }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception err)
            {
                var ExceptionResult = "ReconsiderarIniciativa, Mensaje: " + err.Message.ToString() + "-" + ", Detalle: " + err.StackTrace.ToString();
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult ReintegrarIniciativaDesarrollo(string IniToken)
        {
            if (!@User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Logout", "Login");
            ...
            else
            {
                try
                {
                    ...
                    ORM.Query("CAPEX_REINTEGRAR_DESARROLLO", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
                    ...
                    {
                        resultado = "Reintegrada|" + ErrorCodeSP.Trim();
                    }
                    else
                    {
                        resultado = "Error|" + ((!string.IsNullOrEmpty(ErrorCodeSP) && !string.IsNullOrEmpty(ErrorCodeSP.Trim()) ? ErrorCodeSP.Trim() : "1|ERROR AL REINTEGRAR A DESARROLLO"));
                    }
                    string[] datos = resultado.Split('|');
                    if (datos[0] == "Reintegrada")
                    {
                ...
                {
                    var ExceptionResult = "ReintegrarIniciativaDesarrollo, Mensaje: " + err.Message.ToString() + "-" + ", Detalle: " + err.StackTrace.ToString();
                    CapexInfraestructure.Utilities.Utils.LogError(ExceptionResult);
                    return Json(new { Mensaje = "Error" }, JsonRequestBehavior.AllowGet);
                }
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult ReintegrarIniciativaVisacion(string IniToken)
        {
            if (!@User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Logout", "Login");
            ...
            else
            {
                try
                {
                    ...
                    parametos.Add("PidRol", rol);
                    parametos.Add("ErrorCodeOutput", dbType: System.Data.DbType.String, direction: System.Data.ParameterDirection.Output, size: 200);
                    ORM.Query("CAPEX_REINTEGRAR_VISACION", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
                    var ErrorCodeSP = parametos.Get<string>("ErrorCodeOutput");
                    string resultado = string.Empty;
                    if (!string.IsNullOrEmpty(ErrorCodeSP) && !string.IsNullOrEmpty(ErrorCodeSP.Trim()) && ErrorCodeSP.StartsWith("0"))
                    {
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult ReintegrarIniciativaVisacionAMSA(string IniToken)
        {
            if (!@User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Logout", "Login");
            ...
            else
            {
                try
                {
                    ...
                    parametos.Add("PidRol", rol);
                    parametos.Add("ErrorCodeOutput", dbType: System.Data.DbType.String, direction: System.Data.ParameterDirection.Output, size: 200);
                    ORM.Query("CAPEX_REINTEGRAR_VISACION_AMSA", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
                    var ErrorCodeSP = parametos.Get<string>("ErrorCodeOutput");
                    string resultado = string.Empty;
                    if (!string.IsNullOrEmpty(ErrorCodeSP) && !string.IsNullOrEmpty(ErrorCodeSP.Trim()) && ErrorCodeSP.StartsWith("0"))
                    {
