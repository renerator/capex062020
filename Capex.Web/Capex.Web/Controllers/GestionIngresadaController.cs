    ...
    public class GestionIngresadaController : Controller
    {
                ...
                else
                {
                    ...
                    using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
                    {
                        try
                        {
                            ...
                            {
                                var Iniciativa = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_INGRESADA", new { @usuario = usuarioAux, @periodo = anioIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                            ...
                            {
                                var Iniciativa = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_INGRESADA_2", new { @usuario = usuarioAux, @tipoIniciativa = tipoIniciativaSeleccionado, @periodo = anioIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa != null && Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
                    {
                        ...
                        {
                            objConnection.Open();
                            var categorias = SqlMapper.Query(objConnection, "CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = 6, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                            ...
                            if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                            {
                                ...
                                {
                                    var Iniciativa = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_INGRESADA_2_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
                            ...
                            else
                            {
                                ...
                                {
                                    var Iniciativa = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_INGRESADA_2_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                    if (Iniciativa != null && Iniciativa.Count > 0)
                                    {
                                        ViewBag.Iniciativas = Iniciativa;
                                    }
            ...
            else
            {
                ...
                using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
                {
                    try
                    {
                        ...
                        var tipoIniciativaSeleccionado = Convert.ToString(Session["tipoIniciativaSeleccionado"]);
                        var categorias = SqlMapper.Query(objConnection, "CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = 6, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                        var items = SqlMapper.Query(objConnection, "CAPEX_SEL_CATEGORIA_FILTRO_ITEM_VIGENTES", new { @pagina = 6, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                        ...
                        if (categorias != null && categorias.Count > 0)
                        {
                            foreach (var categoria in categorias)
                            {
                                if (categoria.Id == 52)
                                {
                                    continue;//Id = '52', Nombre = 'AÑO EJERCICIO'
                                }
        ...
        public ActionResult ObtenerListadoRevisores()
        {
            using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
            {
                ...
                {
                    objConnection.Open();
                    var resultado = ViewBag.Revisores = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_REVISORES", commandType: CommandType.StoredProcedure).ToList();
                    return resultado;
                }
                catch (Exception ex)
                {
        ...
        public ActionResult Asignar(string IniToken, string Usuario, string Observacion, string Estado)
        {
            using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
            {
                try
                {
                    ...
                    SqlMapper.Query(objConnection, "CAPEX_INS_GESTION_REVISOR", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
        ...
        public ActionResult AprobarIniciativa(string IniToken, string Usuario)
        {
            using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
            {
                try
                {
                    ...
                    parametos.Add("IniToken", IniToken);
                    parametos.Add("Usuario", Usuario);
                    SqlMapper.Query(objConnection, "CAPEX_INS_GESTION_APROBACION_ADMIN1", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
        ...
        public ActionResult RechazarIniciativa(string IniToken, string Usuario, string Observacion)
        {
            using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
            {
                try
                {
                    ...
                    parametos.Add("Usuario", Usuario);
                    parametos.Add("Observacion", Observacion);
                    SqlMapper.Query(objConnection, "CAPEX_INS_GESTION_RECHAZO_ADMIN1", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
            ...
            else
            {
                using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
                {
                    try
                    {
                        ...
                        SqlMapper.Query(objConnection, "CAPEX_NO_APROBAR_CE", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
