                ...
                else
                {
                    ...
                    using (SqlConnection objConnection = new SqlConnection(CapexIdentity.Utilities.Utils.ConnectionString()))
                    {
                        try
                        {
                            ...
                            {
                                var Iniciativa = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_VISACION", new { @usuario = usuarioAux }, commandType: CommandType.StoredProcedure).ToList();
                            ...
                            {
                                var Iniciativa = SqlMapper.Query(objConnection, "CAPEX_SEL_GESTION_VISACION_2", new { @usuario = usuarioAux, @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
