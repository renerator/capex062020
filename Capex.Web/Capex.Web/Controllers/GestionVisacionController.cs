    ...
    public class GestionVisacionController : Controller
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        ViewBag.Mensajes = ORM.Query("CAPEX_SEL_OBTENER_COMENTARIOS", new { @usuario = usuarioAux }, commandType: CommandType.StoredProcedure).ToList();
                        if (tipoIniciativaSeleccionado.Equals("0"))
                        {
                            var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_VISACION", new { @usuario = usuarioAux }, commandType: CommandType.StoredProcedure).ToList();
                        ...
                        {
                            var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_VISACION_2", new { @usuario = usuarioAux, @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
        ...
        public ActionResult getData(string filtroGetData)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        var categorias = ORM.Query("CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = 4, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                        ...
                        if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                        {
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_VISACION_2_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        else
                        {
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_VISACION_2_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
        ...
        public ActionResult getTreeFilter()
        {
            ...
            try
            {
                var tipoIniciativaSeleccionado = Convert.ToString(Session["tipoIniciativaSeleccionado"]);
                var categorias = ORM.Query("CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = 4, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                var items = ORM.Query("[CAPEX_SEL_CATEGORIA_FILTRO_ITEM_VIGENTES]", new { @pagina = 4, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
        ...
        public ActionResult AprobarIniciativa(string IniToken, string Usuario)
        {
            try
            {
                ...
                ORM.Query("CAPEX_INS_GESTION_APROBACION_SPONSOR", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
        ...
        public ActionResult RechazarIniciativa(string IniToken, string Usuario, string Observacion)
        {
            try
            {
                ...
                ORM.Query("CAPEX_INS_GESTION_RECHAZO_SPONSOR", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
        ...
        public ActionResult NoAprobarIniciativaVisacion(string IniToken)
        {
            ...
            else
            {
                try
                {
                    ...
                    ORM.Query("CAPEX_NO_APROBAR_VISACION", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
