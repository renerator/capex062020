...
namespace Capex.Web.Controllers
{
    ...
    [RoutePrefix("GestionDesarrollo")]
    public class GestionDesarrolloController : Controller
    {
        /* ------------------------------------------------------------------------------------
         * 
         * PMO360
        ...
        ///     
        ///     ----------------------------------------------------------------------------
        ///     CONTROLADOR "GestionDesarrolloController" 
        ///     VERSION     0.0.1
        ///     ----------------------------------------------------------------------------
        ///     PROPOSITO
        ///     ----------------------------------------------------------------------------
        ...
        #region "CONSTRUCTOR"
        public GestionDesarrolloController()
        {
            ////IDENTIFICACION
        ...
        public ActionResult Index()
        {
            ...
            else
            {
                ...
                //
                Session["CAPEX_SESS_VISTA_CONTENEDORA_PADRE"] = "GestionDesarrollo";
                //
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                        {
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO", new { @usuario = usuario }, commandType: CommandType.StoredProcedure).ToList();
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO_2", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        else
                        {
                            if (tipoIniciativaSeleccionado.Equals("0"))
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO", new { @usuario = "" }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO_2", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
        ...
        public ActionResult getData(string filtroGetData)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        var categorias = ORM.Query("CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = 3, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                        ...
                        if (!rol.Contains("Administrador1") && !rol.Contains("Administrador2") && !rol.Contains("Administrador3"))
                        {
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO_2_FILTROS", new { @usuario = usuario, @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        else
                        {
                            if (tipoIniciativaSeleccionado.Equals("0"))
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                            ...
                            {
                                var Iniciativa = ORM.Query("CAPEX_SEL_GESTION_DESARROLLO_2_FILTROS", new { @usuario = "", @tipoIniciativa = tipoIniciativaSeleccionado, @anio = anio, @etapa = etapa, @area = area, @proceso = proceso, @clase = clase, @estadoProyecto = estadoProyecto, @estadoIniciativa = estadoIniciativa, @macroCategoria = macroCategoria, @categoria = categoria, @clasificacionSSO = clasificacionSSO, @nivelIngenieria = nivelIngenieria, @clasificacionRiesgo = clasificacionRiesgo, @gerenciaEjecutora = gerenciaEjecutora, @gerenciaInversion = gerenciaInversion, @estandarSeguridad = estandarSeguridad }, commandType: CommandType.StoredProcedure).ToList();
                                if (Iniciativa.Count > 0)
                                {
                                    ViewBag.Iniciativas = Iniciativa;
                                }
                        ...
                        if (ViewBag.Iniciativas != null && ViewBag.Iniciativas.Count > 0)
                        {
                            ...
                            foreach (var result in ViewBag.Iniciativas)
                            {
                                ...
                                else if (Convert.ToString(@Session["CAPEX_SESS_ROLNOMBRE"]) == "Administrador1")
                                {
                                    ...
                                    selectTrs.Append("<option value = " + Convert.ToChar(34) + "- 1" + Convert.ToChar(34) + " selected>...</option>");
                                    selectTrs.Append("<option value = " + Convert.ToChar(34) + "0" + Convert.ToChar(34) + ">Ver</ option >");
                                    if (Convert.ToString(@Session["CAPEX_SESS_USERNAME"]).Equals(result.UsuarioOwner, StringComparison.OrdinalIgnoreCase))
                                    {
                                        selectTrs.Append("<option value = " + Convert.ToChar(34) + "1" + Convert.ToChar(34) + ">Editar</option>");
                                    }
                                    selectTrs.Append("<option value = " + Convert.ToChar(34) + "2" + Convert.ToChar(34) + ">Adjuntos</ option >");
        ...
        public ActionResult getTreeFilter()
        {
            ...
            try
            {
                var tipoIniciativaSeleccionado = Convert.ToString(Session["tipoIniciativaSeleccionado"]);
                var categorias = ORM.Query("CAPEX_SEL_CATEGORIA_FILTRO_VIGENTES", new { @pagina = 3, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
                var items = ORM.Query("[CAPEX_SEL_CATEGORIA_FILTRO_ITEM_VIGENTES]", new { @pagina = 3, @categoria = tipoIniciativaSeleccionado }, commandType: CommandType.StoredProcedure).ToList();
            ...
            else
            {
                if (rol.Contains("Gestor"))
                {
                    try
                    {
                        ...
                        ORM.Query("CAPEX_DEL_INICIATIVA_DESARROLLO", parametos, commandType: CommandType.StoredProcedure).SingleOrDefault();
